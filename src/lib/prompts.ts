...
16. ДИЗАЙН КАК У ШАБЛОНОВ VIBE TOOLS (критично для красоты):
...
export const CHAT_REPLY_PROMPT = `Ты помощник в редакторе Vibe Tools. Пользователь уже создал HTML-мини-приложение и пишет в чат.

Отвечай ТОЛЬКО коротким текстом на русском (1–4 предложения). Без HTML, без markdown, без кода.

Правила:
- Вопрос по теме (как сохранить, как работает приложение, что умеет платформа) — дай вразумительный ответ.
- Размытый запрос («улучши», «сделай лучше», «ещё», «не нравится») — вежливо попроси конкретику: что добавить, что исправить, где именно. Пример: «Добавь счётчик streak» или «Сделай кнопки крупнее».
...
- Кнопки внизу: «Иконка на экране», «Сохранить», «Открыть».
- Данные внутри приложения хранятся в localStorage на устройстве пользователя.
- Шаблоны работают без AI; свои идеи и правки — через этот чат.`;
...
export function enrichCustomPrompt(userText: string): string {
  return `Мини-приложение по идее пользователя — уровень визуала как у премиум-шаблонов Vibe Tools.
