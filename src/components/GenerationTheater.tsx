...
export function GenerationTheater({
  ...
  return (
    <div className="fixed inset-0 z-50 flex items-center justify-center bg-[#07070f]/95 p-4 backdrop-blur-md">
      <div className="absolute inset-0 overflow-hidden pointer-events-none">
        <div className="absolute left-1/4 top-1/4 h-48 w-48 rounded-full bg-violet-600/20 blur-3xl animate-pulse" />
        <div className="absolute bottom-1/4 right-1/4 h-48 w-48 rounded-full bg-fuchsia-600/15 blur-3xl animate-pulse" />
      </div>

      <div className="relative w-full max-w-lg animate-[fadeUp_.4s_ease-out]">
        ...
        <div className="grid gap-3 md:grid-cols-2">
                  ...
                  <span
                    className={`flex h-5 w-5 shrink-0 items-center justify-center rounded-full text-xs ${
                      i < stepIndex
                        ...
                        : i === stepIndex
                          ? "bg-violet-500/40 text-violet-200 animate-pulse"
          ...
          <div className="rounded-xl border border-violet-500/20 bg-[#0d1117] p-3 font-mono text-xs shadow-inner shadow-violet-500/10">
            ...
            <pre className="min-h-[72px] overflow-hidden text-emerald-400/90">
              <code>
                {typedCode}
                <span className="animate-pulse text-violet-400">|</span>
              </code>
            </pre>
            {script.codeLines.slice(0, codeIndex).map((line, i) => (
              <div
                key={i}
                className="mt-1 truncate text-xs text-white/20"
              >
                {line}
              </div>
            ))}
